...
`hippo.superbalist.com`). Gate #1: a `superbalist.com` Google Workspace account.
...
| Mode | Mechanism | Use |
|------|-----------|-----|
| `iap` | Verify GCP IAP's signed `x-goog-iap-jwt-assertion` header (signature against Google's public keys + audience check), extract email. No login UI or sessions in-app — IAP already did that at the load balancer. | Superbalist production |
...
route logic runs. `HIPPO_ALLOWED_DOMAIN` (e.g. `superbalist.com`) is enforced in both real
...
- **Docker**: multi-stage Dockerfile (build `ui/dist` → `uv` install → one image serving
  ...
  Superbalist* (manifests, IAP audience config, git-sync sidecar, probes): IAP and the
...
**Deploy at Superbalist** (k8s manifests, IAP wiring, git-sync sidecar) · **MCP OAuth for
