...
def test_resolve_role_first_timer_is_developer(tmp_path):
    ...
    assert resolve_role(store, settings, "new.person@superbalist.com") == "developer"


def test_resolve_role_admin_bootstrap_wins(tmp_path):
    store = _store(tmp_path)
    settings = Settings(allowed_domain="superbalist.com", admin_emails="boss@superbalist.com")
    assert resolve_role(store, settings, "Boss@Superbalist.com") == "admin"
